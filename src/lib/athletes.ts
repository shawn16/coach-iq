...
// Delete an athlete from the database (soft delete)
export async function deleteAthlete(id: number): Promise<boolean> {
  try {
    await prisma.$softDelete('Athlete', { id });
...
// Permanently delete an athlete from the database (bypasses soft delete)
export async function forceDeleteAthlete(id: number): Promise<boolean> {
  try {
    await prisma.$forceDelete('Athlete', { id });
    ...
    console.error("Error force deleting athlete:", error);
    return false;
  }
}
